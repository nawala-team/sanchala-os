...
class ContentFilter:
    ...
    def block_site(self, domain):
        ...
        if domain not in cfg["blocked_sites"]:
            cfg["blocked_sites"].append(domain)
    ...
    def apply_blocks(self):
        ...
        with open(temp_file) as f:
            subprocess.run(["sudo", "tee", "-a", "/etc/hosts"], stdin=f, stdout=subprocess.DEVNULL)
        os.remove(temp_file)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if cf.block_site(sys.argv[2]):
            print(f"Blocked {sys.argv[2]}")
    elif sys.argv[1] == "list":
        for s in cf.load_config()["blocked_sites"]:
            print(f"  {s}")
    elif sys.argv[1] == "status":
        print(f"Enabled: {cf.load_config()['enabled']}")
